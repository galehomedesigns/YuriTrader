...
All scripts live in `/data/skills/kraken-cli/scripts/`. They share a common
...
scripts in `/data/skills/crypto-trading/scripts/yuri/`. If Tony asks about yuri's
...
### System status
```bash
node /data/skills/kraken-cli/scripts/system-status.mjs
```
...
```bash
node /data/skills/kraken-cli/scripts/ticker.mjs XBTUSD
...
### OHLC candles
```bash
node /data/skills/kraken-cli/scripts/ohlc.mjs XBTUSD          # default 60m
node /data/skills/kraken-cli/scripts/ohlc.mjs XBTUSD 240      # 4h candles
```
...
### Order book
```bash
node /data/skills/kraken-cli/scripts/orderbook.mjs XBTUSD 10
```
...
### Balance
```bash
node /data/skills/kraken-cli/scripts/balance.mjs
```
...
### Open orders
```bash
node /data/skills/kraken-cli/scripts/open-orders.mjs
```
...
```bash
node /data/skills/kraken-cli/scripts/closed-orders.mjs
node /data/skills/kraken-cli/scripts/closed-orders.mjs --ofs=50
```
...
```bash
node /data/skills/kraken-cli/scripts/trades-history.mjs
```
...
```bash
node /data/skills/kraken-cli/scripts/add-order.mjs --pair=XBTUSD --type=buy --ordertype=market --volume=0.0001
```
...
node /data/skills/kraken-cli/scripts/add-order.mjs --pair=XBTUSD --type=buy --ordertype=limit --volume=0.0001 --price=80000 --validate=false
```
...
### Cancel an order
```bash
node /data/skills/kraken-cli/scripts/cancel-order.mjs <txid>
```
